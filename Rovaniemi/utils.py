...
def get_planned_path(longest=150):
    planned_path = {}
    traci.start(sumoCmd)
    while shouldContinueSim():
        traci.simulationStep()
        for veh in traci.vehicle.getIDList():
            ...
            for link in traci.vehicle.getNextLinks(veh):
                ...
                for point in traci.lane.getShape(lane):
                    if point[0] == points[-1][0] and point[1] == points[-1][1]:
                        continue
                    norm = np.linalg.norm(np.array(point) - np.array(points[-1]))
                    pieces = norm // longest + 1
                    if pieces > 1:
                        for i in range(1, int(pieces)):
                            new_point = np.array(points[-1]) + (np.array(point) - np.array(points[-1])) * i / pieces
                            points.append([new_point[0], new_point[1]])
                    points.append([point[0], point[1]])
